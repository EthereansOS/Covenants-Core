...
describe("WUSD", () => {
    ...
    async function encodeNewAMMV2Params() {
        var lPTokens = [
        ...
        for(var lp of lPTokens) {
            ...
            console.log(tokenNames.join(' - '), info[0], lp);
            (data[ammAddress] = data[ammAddress] || [ammAddress, []])[1].push(lp);
        }
        return allowedAMMS = Object.values(data);
        var types = ['tuple(address,address[])[]'];
        var params = [allowedAMMS];
        var encoded = abi.encode(types, params);
        return encoded;
    ...
    it("Final AMMS and new URI", async () => {
        ...
        var allowedAMMSCopy = await encodeNewAMMV2Params();
