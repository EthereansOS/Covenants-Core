...
contract FarmMainMinStake is IFarmMain, BlockRetriever {
    ...
    /** @dev initializes the farming contract.
      * uniswapV3NonfungiblePositionManager Uniswap V3 Nonfungible Position Manager address.
      * extension extension address.
      * extensionInitData lm extension init payload.
      * rewardTokenAddress address of the reward token.
      * farmingSetupInfosBytes optional initial farming Setup Info
    ...
    function finalFlush(address[] calldata tokens, uint256[] calldata amounts) public {
        ...
        (,,, address receiver,) = IFarmExtension(host).data();
    ...
    function _withdrawReward(uint256 positionId, uint128 liquidityToRemove, uint256 amount0Min, uint256 amount1Min, bytes memory burnData) private {
        ...
        _retrieveGen2LiquidityAndFees(positionId, farmingSetup.objectId, reward, farmingPosition.uniqueOwner, liquidityToRemove, amount0Min, amount1Min, burnData);
    ...
    function _addLiquidity(uint256 setupIndex, FarmingPositionRequest memory request) private returns(uint128 liquidityAmount) {
        ...
        if(_setupPositionsCount[setupIndex] == 0 && _setups[setupIndex].objectId == 0) {
            ...
            uint256 objectId;
            (objectId, liquidityAmount, amount0, amount1) = abi.decode(IMulticall(address(nonfungiblePositionManager)).multicall{ value: ethValue }(data)[0], (uint256, uint128, uint256, uint256));
    ...
    function _giveBack(uint256 amount) private {
        ...
        if (_rewardTokenAddress == address(0)) {
            IFarmExtension(host).backToYou{value : amount}(amount);
        } else {
            _safeApprove(_rewardTokenAddress, host, amount);
            IFarmExtension(host).backToYou(amount);
        }
    }
    ...
    function _ensureTransfer(uint256 amount) private returns(bool) {
        ...
        try IFarmExtension(host).transferTo(amount) {} catch {}
    ...
    function _retrieveGen2LiquidityAndFees(uint256 positionId, uint256 objectId, uint256 reward, address recipient, uint128 liquidityToRemove, uint256 amount0Min, uint256 amount1Min, bytes memory burnData) private {
